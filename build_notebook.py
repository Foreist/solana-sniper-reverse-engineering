...
for c in FEATURES:
    ...
    if a and np.isfinite(a) and np.isfinite(b) and (b / a > 2 or b / a < 0.5):
        shift.append((c, round(float(a), 3), round(float(b), 3)))
print(f"  columns shifting > 2x: {shift if shift else 'NONE'}\\n")
...
def at_recall(y, s, target=0.05):
    p, r, _ = precision_recall_curve(y, s)
    ok = r >= target
    return p[ok].max() if ok.any() else np.nan
...
for name, col in [("GBDT", "score_gbdt"), ("Logistic", "score_logit")]:
    ...
    rows.append({"model": name,
                 ...
                 "precision @ 5% recall": at_recall(y, s)})
